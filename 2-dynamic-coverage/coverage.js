  ...
  .then(() => {
    CDP().then(
      async client => {
        ...
        CSS.styleSheetAdded(({ header }) => styles.push(header));
        ...
        // Use two rAF's: https://github.com/cyrus-and/chrome-remote-interface/issues/238#issuecomment-322902250
        // Code: https://github.com/cyrus-and/chrome-remote-interface/issues/238#issuecomment-323134653
        await Runtime.evaluate({
          expression: `
            new Promise(resolve =>
        ...
        let { result: scriptCoverages } = await Profiler.takePreciseCoverage();
        ...
        scriptCoverages.filter(({ url }) => url).forEach(script => {
          resources.push({
            id: script.scriptId,
            file: script.url,
